...
class Node:
    ...
    - all([self in self.channels[address].endpoints for address in self.channels])
    """
    address: NodeAddress
    ...
    def find_paths(self, destination: NodeAddress, visited: set[Node]) -> list[list[Channel]]:
        ...
        if self.address == destination:
            ...
            for neighbour_address in self.channels:
                connected_channel = self.channels[neighbour_address]
                neighbour_node = connected_channel.get_other_endpoint(self)
                path = [connected_channel]
                if neighbour_node not in new_visited:
                    neighbour_node.connect_paths(destination, new_visited, path, paths)

            return paths

    def connect_paths(self, destination: NodeAddress, visited: set[Node], path: list[Channel],
                      paths: list[list[Channel]]) -> None:
        """A helper function for find_paths that append each possible path from the node's neighbour to the list.
        """
        neighbour_paths = self.find_paths(destination, visited)
        for n_path in neighbour_paths:
            paths.append(path + n_path)
    ...
    def transmit_packet(self, packet: Packet) -> list[NodeAddress]:
        ...
        Implementation Hint:
            - You can use the packet.next_stop attribute to keep track of which nodes the packet
              is visiting
        """
        nodes_pathway = [packet.source]
        channel_selected = self.route_packet(packet.source, packet)
        channel_selected.add_packet(packet, self._nodes[packet.source])

        # while packet hasn't reached its destination, we keep moving forward
        while packet.next_stop is not None and packet.next_stop.address != packet.destination:
            nodes_pathway.append(packet.next_stop.address)
            channel_selected = self.activate_channel(channel_selected)

        nodes_pathway.append(packet.destination)

        return nodes_pathway
